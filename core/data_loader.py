...
# Координаты оставляю в точности как у вас (чтобы не занимать место в ответе, они такие же, как были)
TOPOLOGY_POS = {
...
def load_network_data(df_req=None, df_cap=None, topology_override=None):
    ...
    # Строим граф для проверки достижимости (ПУНКТ 6)
        ...
        for _, row in df_req.iterrows():
            ...
            try:
                ...
                if src and dst and vol > 0:
                    ...
                    # Проверяем, есть ли путь вообще в текущей топологии
    ...
    # ПУНКТ 1: Все связи из топологии по умолчанию безлимитны. Лишние игнорируются.
    ...
    if df_cap is not None:
        for _, row in df_cap.iterrows():
            try:
                ...
                # Пытаемся применить ограничение только если оно ложится на существующие ребра топологии
                try:
                    ...
                    pass  # Лишние ребра просто игнорируются
            except (ValueError, TypeError):
                continue
