...
from core.data_loader import load_network_data, get_logical_edges, get_current_topology, normalize_node_strictly, \
    parse_float_safely
...
st.sidebar.markdown("---")
algo = st.sidebar.radio("🤖 Алгоритм оптимизации",
...
if st.session_state.df_req is not None and st.session_state.df_cap is not None:
    ...
    with col_e1:
        ...
        df_req_edited = st.data_editor(st.session_state.df_req, num_rows="dynamic", use_container_width=True,
                                       on_change=reset_results, key="editor_req")
    with col_e2:
        st.markdown("**Таблица 1.2: Ограничения пропускной способности**")
        df_cap_edited = st.data_editor(st.session_state.df_cap, num_rows="dynamic", use_container_width=True,
    ...
    try:
        ...
        if invalid_reqs:
            ...
            with st.expander("Детали недостижимых узлов"):
    ...
    if st.session_state.res:
        ...
        m2.metric("Доставленная мощность", f"{total_delivered:.2f} кВт",
                  delta=f"{total_delivered - total_requested:.2f}")
        ...
        for (u, v), cap in caps.items():
            ...
            if load > 0.1:
                edge_list.append(
                    {'edge': (u, v), 'label': f"{u} → {v} (Загрузка: {load / cap:.1%}, {load:.2f}/{cap:.2f} кВт)",
                     'ratio': load / cap})

        edge_list = sorted(edge_list, key=lambda x: x['ratio'], reverse=True)
        sel_edge_label = st.selectbox("Анализ вклада в нагрузку участка:",
                                      ["-- выберите участок --"] + [e['label'] for e in edge_list])

        available_reqs = res['request_flows'].keys()
        req_to_opt = {rk: f"{rk[0]} → {rk[1]} ({res['delivered'].get(rk, 0):.2f}/{reqs.get(rk, 0):.2f} кВт)" for rk in
                      available_reqs}
        ...
        if sel_edge_label != "-- выберите участок --":
            ...
            breakdown = [{"key": rk, "label": req_to_opt[rk], "flow": f.get(target_edge, 0)} for rk, f in
                         res['request_flows'].items() if f.get(target_edge, 0) > 0.001]
            with st.expander(f"Детализация участка {target_edge[0]} → {target_edge[1]}", expanded=True):
                ...
                for b in breakdown:
                    st.checkbox(f"{b['label']} | Вклад: **{b['flow']:.2f} кВт**", value=b['label'] in cur_sel,
                                key=f"chk_{b['key']}", on_change=toggle_request, args=(b['label'],))

        st.subheader("🗺️ Геопространственная визуализация")
        selected_opts = st.multiselect("🔍 Фильтр по заявкам (подсветка путей):", options=list(req_to_opt.values()),
                                       key="req_selector")
        selected_load = None
        if selected_opts:
            selected_load = {}
        ...
        stats = [{"Источник": s, "Потребитель": d, "Заявка": r, "Факт": res['delivered'].get((s, d), 0),
                  "Процент": (res['delivered'].get((s, d), 0) / r * 100) if r > 0 else 100} for (s, d), r in
                 reqs.items()]
        ...
        total_row = pd.DataFrame([{
            "От (Узел)": "ИТОГО",
            "До (Узел)": "ДОСТАВЛЕНО",
            "Мощность (кВт)": round(total_delivered, 2)
        }])
        df_export = pd.concat([df_export, total_row], ignore_index=True)

        buffer = io.BytesIO()
        with pd.ExcelWriter(buffer, engine='openpyxl') as writer:
            df_export.to_excel(writer, index=False, sheet_name='Нагрузка по ребрам')
        buffer.seek(0)
        ...
        with c_t1:
            ...
            st.download_button(
                label="📥 Скачать решение Задания 1 (Excel)",
                data=buffer,
                file_name="zadanie_1_edges.xlsx",
        ...
        try:
            ...
            st.warning("Недостаточно данных для отрисовки схемы.")