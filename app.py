...
def toggle_request(opt_label: str) -> None:
    """
    Переключает выбор заявки в мультиселекторе (используется в инспекторе).
    
    Args:
        opt_label: Строковая метка заявки.
    """
...
def clean_value(val: Any) -> float:
    """
    Очищает и конвертирует строковые значения из таблиц в float.
    Обрабатывает пробелы и запятые.
...
def process_pdf_req(file: Any) -> pd.DataFrame:
    """
    Парсит PDF-файл с таблицей заявок (Таблица 1.1).
...
def process_pdf_cap(file: Any) -> pd.DataFrame:
    """
    Парсит PDF-файл с таблицей лимитов (Таблица 1.2).
...
def apply_new_data(df_r: pd.DataFrame, df_c: pd.DataFrame) -> None:
    ...
    Args:
        df_r: Датафрейм заявок.
        df_c: Датафрейм ограничений.
    """
...
if st.sidebar.button("📁 Применить загруженные файлы"):
    ...
    st.sidebar.success("Файлы загружены!")
...
if st.session_state.df_req is not None and st.session_state.df_cap is not None:
    ...
    # Валидация топологии в реальном времени
    ...
    if st.session_state.res:
        ...
        # Логика инспектора (подсветка вкладов)
        ...
        # Метрики справедливости (Fairness)
